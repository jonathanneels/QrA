...
process
  .on("unhandledRejection", (reason, p) => {
    //https://shapeshed.com/uncaught-exceptions-in-node/ & https://stackoverflow.com/questions/40867345/catch-all-uncaughtexception-for-node-js-app
...
function launchServer() {
  fs.open(
    __dirname + "\\static\\Augme\\storedMindFiles\\customNames_linker.txt",
    "r",
    function (err, fd) {
      if (err) {
        fs.writeFile(
          __dirname +
            "\\static\\Augme\\storedMindFiles\\customNames_linker.txt",
          "",
          function (err) {
            if (err) {
              console.log(err);
    ...
    .createServer((req, res) => {
      ...
      if (req.method == "POST") {
          ...
          try {
            ...
            req.on("end", function () {
              ...
              try {
                console.log(body);
                var dirName =
                  __dirname + "\\static\\Augme\\storedMindFiles\\" + body;
          ...
          try {
            ...
            req.on("end", function () {
              ...
              for (let i = 0; i < readFileLines.length; i++) {
                if (
                  readFileLines[i].indexOf(body) >= 0 &&
                  readFileLines[i].split("|")[0].trim() == body
                ) {
          ...
          try {
            ...
            req.on("end", function () {
              ...
              try {
                ...
                var dirName =
                  __dirname + "\\static\\Augme\\storedMindFiles\\" + dayNow;
        ...
        if (feedbackUrl.trim() === "/") {
          ...
          res.end(
            decodeURIComponent(
              feedbackUrl.trim().toLowerCase().replace("/mirror/", "").trim(),
            ),
          );
          ...
          var path =
            __dirname +
            "\\static\\Augme\\storedMindFiles\\" +
            decodeURIComponent(feedbackUrl.trim().toLowerCase()).slice(10);
          ...
          fs.readFile(
            __dirname + "/static/Augme/AugmeHomePage.html",
            function (err, data) {
              res.writeHead(200);
              res.end(data);
            },
          ...
          feedbackUrl.trim().toLowerCase().startsWith("/mindarcompiler")
        ) {
          fs.readFile(
            __dirname + "/static/Augme/compiler_to_product_oneshot.html",
            function (err, data) {
              res.writeHead(200);
              res.end(data);
            },
          ...
          fs.readFile(
            __dirname + "/static/Augme/iframe_mindAR_production.html",
            function (err, data) {
              res.writeHead(200);
              res.end(data);
            },
          ...
          fs.readFile(
            __dirname + "/QrAndQRACOMBINED.html",
            function (err, data) {
              res.writeHead(200);
              res.end(data);
            },
          );
        } else if (feedbackUrl.trim().startsWith("/Q")) {
          // databased page. Info after the Q
          fs.readFile(__dirname + "/iframe.html", function (err, data) {
            res.writeHead(200);
            res.end(data);
          });
        } else if (
          feedbackUrl.trim().startsWith("/read") ||
          feedbackUrl.trim() == "/qr"
        ) {
          res.writeHead(200, {
            "Content-Type": "text/html",
          ...
          fs.readFile(
            __dirname + "/static/QrA_Maker/index.html",
            function (err, data) {
              res.writeHead(200);
              res.end(data);
            },
          ...
          fs.readFile(
            __dirname + "/static/QrA_Maker/indexVR.html",
            function (err, data) {
              res.writeHead(200);
              res.end(data);
            },
          ...
          var feedbackUrlParts = decodeString
            .replace("/memo", "")
            .trim()
            .split("|");
          if (feedbackUrlParts.length == 2) {
            ...
            memoNote = {
              user: feedbackUrlParts[0].trim(),
              text: feedbackUrlParts[1].trim(),
              date: dateTimeNow(),
            };
            memoList.push(memoNote);
            res.writeHead(200, { "Content-Type": "text/html" }); //res.writeHead(200,{"Content-Type" : "text/plain"});
            res.end(
              "Success!!<br>Call your memo: /call" + feedbackUrlParts[0].trim(),
            ); //res.end("Hello World<br><b> w</b>");
          } else {
            res.writeHead(200, { "Content-Type": "text/html" });
          ...
          fs.readFile(__dirname + feedbackUrl, function (err, data) {
